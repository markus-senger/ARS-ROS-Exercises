...
def odometry_publisher():
    ...
    with open('src/my-odometry-1-2/trajectoryTextfiles/trajectory_my_odom_real.txt', 'r') as file:
        for line in file:
    ...
    with open('src/my-odometry-1-2/trajectoryTextfiles/trajectory_odom_real.txt', 'r') as file: 
        for line in file:
            parts = line.strip().split() 
            if len(parts) == 7:
                odometry_msg = Odometry()
